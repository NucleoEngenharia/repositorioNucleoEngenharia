...
public class Commom {
	...
	public static AcessosUsuario getAcessoUsuarioLogado(){
		ExternalContext externalContext = FacesContext.getCurrentInstance().getExternalContext();
		HttpSession session = (HttpSession) externalContext.getSession(true);
		LoginBean loginBean = (LoginBean) session.getAttribute("loginBean");
		return loginBean.getAcesso();
	}
	...
	public static Object lookup(String className) {
		...
		try {
			...
			try {
				classeDAO = ic.lookup("java:global/Nucleo.Web/" + className);
			} catch (NamingException e) {
				try {
					classeDAO = ic.lookup("java:global/Nucleo/Nucleo.Medicao/" + className);
