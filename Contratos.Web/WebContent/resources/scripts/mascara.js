...
function cep(v) {
	...
	v = v.replace(/(\d{2})(\d)/, "$1.$2"); // Coloca um ponto entre o terceiro
											// e o quarto dígitos
	v = v.replace(/(\d{3})(\d{1,3})$/, "$1-$2"); // Coloca um hífen entre o
												// terceiro e o quarto dígitos
	return v;
}
...
function data(v) {
	...
	v = v.replace(/\.(\d{2})(\d)/, ".$1/$2"); // Coloca uma barra entre o
												// oitavo e o nono dígitos
	v = v.replace(/(\d{2})(\d)/, "$1/$2"); // Coloca um hífen depois do bloco
											// de quatro dígitos
	return v;
}
...
function porcentagem(v) {
	v = v.replace(/\D/g, ""); // permite digitar apenas numero
	v = v.replace(/(\d{1})(\d{1,6})$/, "$1,$2"); // coloca virgula antes dos
	return v;
}
